...
# setup.sh — Universal setup script for heterogenous-mobile-computing
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/heterogenous-mobile-computing/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
# ── Main ──
main() {
    printf "%s=== heterogenous-mobile-computing Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/heterogenous-mobile-computing.git"
        info "Then: cd heterogenous-mobile-computing && ./setup.sh"
        exit 1
    fi
